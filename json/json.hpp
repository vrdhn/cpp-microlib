...
template <typename T> bool parse(const char *str, T &out) {
	stream s(str);
	return _parse(s, out);
}
...
bool _parse(stream &s, std::string &out) {
	s.skip_ws();
	if (!s.is_char('"'))
...
template <typename E> bool _parse(stream &s, std::vector<E> &out) {
	out.clear();
	if (!s.array_begin())
		return false;
...
template <typename... F> bool _parse(stream &s, std::tuple<F...> &out) {
	if (!s.map_begin())
		return false;
	...
	do {
		...
		if (!s.is_colon())
			return false;
		if (!_parse<std::tuple_size<std::tuple<F...>>::value - 1, F...>(
			s, key, out))
			return false;
	} while (s.is_comma());
...
bool _parse(stream &s, std::string &key, std::tuple<F...> &out) {
